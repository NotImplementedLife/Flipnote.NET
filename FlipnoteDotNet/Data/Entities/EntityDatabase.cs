...
namespace FlipnoteDotNet.Data.Entities
{
    public class EntityDatabase
    {
        ...
        public IEntityReference<Entity> Create(Type entityType) 
        {
            if (entityType == null)
                throw new ArgumentNullException(nameof(entityType));
            if (entityType != typeof(Entity) && !entityType.IsSubclassOf(typeof(Entity)))
                throw new ArgumentException("Argument must be Entity or a derived type", nameof(entityType));

            var eId = Entities.Count;
            var entityData = new EntityData(eId, entityType);
            Entities.Add(entityData);
            return FindById(eId);
        }

        public IEntityReference<Entity> Create(Type entityType, int id)
        {
            if (entityType == null)
                throw new ArgumentNullException(nameof(entityType));
            if (entityType != typeof(Entity) && !entityType.IsSubclassOf(typeof(Entity)))
            ...
            var entityData = new EntityData(id, entityType);
                ...
                foreach(var propEntry in Properties)
                {
                    ...
                    else if(prop is EntityListProperty entityListProperty)
                    {
                        ...
                        foreach (var id in entityListProperty.EntityIds)
                            list.AddEntity(db.GetEntityDataById(id).BuildEntity(db, timestamp));
